...
const createCollege = async (req, res) => {
  try {
    ...
    if (Object.keys(data) == 0) return res.status(400).send({ status: false, msg: "Please Provide Some Data" });

    if (!validator.isValidName(name)) return res.status(404).send({ status: false, msg: "Please Provide a Valid Name" });

    if (!fullName || fullName.trim().length === 0) return res.status(404).send({ status: false, msg: "Please Provide a Valid Full Name" });

    if (!logoLink) return res.status(404).send({ status: false, msg: "Please Provide a LogoLink" });

    const validateLogoLink = url.isUri(logoLink);

    if (!validateLogoLink) return res.status(404).send({ status: false, msg: "Please Provide a Valid Link" });
...
const collegeDetails = async (req, res) => {
  try {
    ...
    if (Object.keys(data) == 0) return res.status(400).send({ status: false, msg: "Please Provide a College Name" });
